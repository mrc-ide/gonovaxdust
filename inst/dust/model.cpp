...
template <typename real_type, typename container>
HOSTDEVICE real_type odin_sum2(const container x, int from_i, int to_i, int from_j, int to_j, int dim_x_1);
template <typename real_type, typename container>
HOSTDEVICE real_type odin_sum3(const container x, int from_i, int to_i, int from_j, int to_j, int from_k, int to_k, int dim_x_1, int dim_x_12);
...
public:
  ...
  struct shared_type {
    ...
    real_type enr;
    real_type epsilon;
    std::vector<real_type> eta_h_step;
    std::vector<real_type> eta_l_step;
    real_type exr;
    real_type fix_pop;
    ...
    std::vector<real_type> initial_cum_diag_a;
    std::vector<real_type> initial_cum_diag_s;
    std::vector<real_type> initial_cum_incid;
    std::vector<real_type> initial_cum_screened;
    std::vector<real_type> initial_cum_treated;
    std::vector<real_type> initial_cum_vaccinated;
    std::vector<real_type> initial_diag_a;
    std::vector<real_type> initial_diag_s;
    real_type initial_entrants;
    std::vector<real_type> initial_eta;
    std::vector<real_type> initial_incid;
    std::vector<real_type> initial_lambda;
    real_type initial_leavers;
    std::vector<real_type> initial_screened;
    real_type initial_time;
    std::vector<real_type> initial_treated;
    std::vector<real_type> initial_vaccinated;
    real_type mu;
    int n_group;
    int n_vax;
    real_type nu;
    ...
    std::vector<real_type> r_ST;
    std::vector<real_type> r_T;
    std::vector<real_type> r_TU;
    real_type rho;
    real_type sigma;
    real_type steps_per_year;
    std::vector<real_type> u_vbe;
    std::vector<real_type> u_vod;
    std::vector<real_type> u_vos;
    std::vector<real_type> vax_step;
  ...
  struct internal_type {
    ...
    std::vector<real_type> r_UI;
    std::vector<real_type> r_UU;
  ...
  size_t size() {
    return shared->dim_A + shared->dim_I + shared->dim_N + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_cum_diag_a + shared->dim_cum_diag_s + shared->dim_cum_incid + shared->dim_cum_screened + shared->dim_cum_treated + shared->dim_cum_vaccinated + shared->dim_diag_a + shared->dim_diag_s + shared->dim_eta + shared->dim_incid + shared->dim_lambda + shared->dim_screened + shared->dim_treated + shared->dim_vaccinated + 4;
  }
  std::vector<real_type> initial(size_t step) {
    std::vector<real_type> state(shared->dim_A + shared->dim_I + shared->dim_N + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_cum_diag_a + shared->dim_cum_diag_s + shared->dim_cum_incid + shared->dim_cum_screened + shared->dim_cum_treated + shared->dim_cum_vaccinated + shared->dim_diag_a + shared->dim_diag_s + shared->dim_eta + shared->dim_incid + shared->dim_lambda + shared->dim_screened + shared->dim_treated + shared->dim_vaccinated + 4);
  ...
  HOST void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    const real_type beta = state[3];
    const real_type * eta = state + 4;
    ...
    const real_type * T = state + shared->offset_variable_T;
    const real_type * N = state + shared->offset_variable_N;
    const real_type * lambda = state + shared->offset_variable_lambda;
    const real_type * cum_incid = state + shared->offset_variable_cum_incid;
    const real_type * cum_diag_a = state + shared->offset_variable_cum_diag_a;
    const real_type * cum_diag_s = state + shared->offset_variable_cum_diag_s;
    const real_type * cum_treated = state + shared->offset_variable_cum_treated;
    const real_type * cum_screened = state + shared->offset_variable_cum_screened;
    const real_type * cum_vaccinated = state + shared->offset_variable_cum_vaccinated;
    const real_type * incid = state + shared->offset_variable_incid;
    const real_type * diag_a = state + shared->offset_variable_diag_a;
    const real_type * diag_s = state + shared->offset_variable_diag_s;
    const real_type * treated = state + shared->offset_variable_treated;
    const real_type * screened = state + shared->offset_variable_screened;
    const real_type * vaccinated = state + shared->offset_variable_vaccinated;
    const real_type entrants = state[1];
    const real_type leavers = state[2];
    ...
    real_type vax_switch = (static_cast<int>(step) >= shared->dim_vax_step ? shared->vax_step[shared->dim_vax_step - 1] : shared->vax_step[step + 1 - 1]);
    for (int i = 1; i <= shared->dim_Np; ++i) {
      internal.Np[i - 1] = odin_sum2<real_type>(N, i - 1, i, 0, shared->dim_N_2, shared->dim_N_1) * shared->p[i - 1];
    }
    ...
    for (int i = 1; i <= shared->dim_n_I_1; ++i) {
      for (int j = 1; j <= shared->dim_n_I_2; ++j) {
        internal.n_I[i - 1 + shared->dim_n_I_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, I[shared->dim_I_1 * (j - 1) + i - 1], 1 - std::exp(- shared->r_I[shared->dim_r_I_1 * (j - 1) + i - 1] * shared->dt));
      }
    }
    for (int i = 1; i <= shared->dim_prop_C; ++i) {
      internal.prop_C[i - 1] = odin_sum2<real_type>(internal.C.data(), i - 1, i, 0, shared->dim_C_2, shared->dim_C_1) / (real_type) odin_sum2<real_type>(N, i - 1, i, 0, shared->dim_N_2, shared->dim_N_1);
    }
    ...
    for (int i = 1; i <= shared->dim_foi_LH; ++i) {
      internal.foi_LH[i - 1] = internal.prop_C[i - 1] * internal.Np[i - 1] / (real_type) odin_sum1<real_type>(internal.Np.data(), 0, shared->dim_Np);
    }
    for (int i = 1; i <= shared->dim_n_A_1; ++i) {
      for (int j = 1; j <= shared->dim_n_A_2; ++j) {
        internal.n_A[i - 1 + shared->dim_n_A_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, A[shared->dim_A_1 * (j - 1) + i - 1], 1 - std::exp(- internal.r_A[shared->dim_r_A_1 * (j - 1) + i - 1] * shared->dt));
      }
    }
    for (int i = 1; i <= shared->dim_n_Iw_1; ++i) {
      for (int j = 1; j <= shared->dim_n_Iw_2; ++j) {
        internal.n_Iw[i - 1 + shared->dim_n_Iw_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, I[shared->dim_I_1 * (j - 1) + i - 1] - internal.n_I[shared->dim_n_I_1 * (j - 1) + i - 1], 1 - std::exp(shared->D[j - 1] * shared->dt));
      }
    }
    for (int i = 1; i <= shared->dim_n_Ix_1; ++i) {
      for (int j = 1; j <= shared->dim_n_Ix_2; ++j) {
        internal.n_Ix[i - 1 + shared->dim_n_Ix_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, internal.n_I[shared->dim_n_I_1 * (j - 1) + i - 1], shared->exr / (real_type) shared->r_I[shared->dim_r_I_1 * (j - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_S_1; ++i) {
      for (int j = 1; j <= shared->dim_n_S_2; ++j) {
        internal.n_S[i - 1 + shared->dim_n_S_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, S[shared->dim_S_1 * (j - 1) + i - 1], 1 - std::exp(- shared->r_S[shared->dim_r_S_1 * (j - 1) + i - 1] * shared->dt));
      }
    }
    for (int i = 1; i <= shared->dim_n_T_1; ++i) {
      for (int j = 1; j <= shared->dim_n_T_2; ++j) {
        internal.n_T[i - 1 + shared->dim_n_T_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, T[shared->dim_T_1 * (j - 1) + i - 1], 1 - std::exp(- shared->r_T[shared->dim_r_T_1 * (j - 1) + i - 1] * shared->dt));
      }
    }
    for (int i = 1; i <= shared->dim_n_U_1; ++i) {
      for (int j = 1; j <= shared->dim_n_U_2; ++j) {
        internal.n_U[i - 1 + shared->dim_n_U_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, U[shared->dim_U_1 * (j - 1) + i - 1], 1 - std::exp(- internal.r_U[shared->dim_r_U_1 * (j - 1) + i - 1] * shared->dt));
      }
    }
    for (int i = 1; i <= shared->dim_n_Aw_1; ++i) {
      for (int j = 1; j <= shared->dim_n_Aw_2; ++j) {
        internal.n_Aw[i - 1 + shared->dim_n_Aw_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, A[shared->dim_A_1 * (j - 1) + i - 1] - internal.n_A[shared->dim_n_A_1 * (j - 1) + i - 1], 1 - std::exp(shared->D[j - 1] * shared->dt));
      }
    }
    for (int i = 1; i <= shared->dim_n_Ax_1; ++i) {
      for (int j = 1; j <= shared->dim_n_Ax_2; ++j) {
        internal.n_Ax[i - 1 + shared->dim_n_Ax_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, internal.n_A[shared->dim_n_A_1 * (j - 1) + i - 1], shared->exr / (real_type) internal.r_A[shared->dim_r_A_1 * (j - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_IS_1; ++i) {
      for (int j = 1; j <= shared->dim_n_IS_2; ++j) {
        internal.n_IS[i - 1 + shared->dim_n_IS_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, internal.n_I[shared->dim_n_I_1 * (j - 1) + i - 1] - internal.n_Ix[shared->dim_n_Ix_1 * (j - 1) + i - 1], (1 - shared->ves[j - 1]) * shared->psi);
      }
    }
    for (int i = 1; i <= shared->dim_n_Sw_1; ++i) {
      for (int j = 1; j <= shared->dim_n_Sw_2; ++j) {
        internal.n_Sw[i - 1 + shared->dim_n_Sw_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, S[shared->dim_S_1 * (j - 1) + i - 1] - internal.n_S[shared->dim_n_S_1 * (j - 1) + i - 1], 1 - std::exp(shared->D[j - 1] * shared->dt));
      }
    }
    for (int i = 1; i <= shared->dim_n_Sx_1; ++i) {
      for (int j = 1; j <= shared->dim_n_Sx_2; ++j) {
        internal.n_Sx[i - 1 + shared->dim_n_Sx_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, internal.n_S[shared->dim_n_S_1 * (j - 1) + i - 1], shared->exr / (real_type) shared->r_S[shared->dim_r_S_1 * (j - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_Tw_1; ++i) {
      for (int j = 1; j <= shared->dim_n_Tw_2; ++j) {
        internal.n_Tw[i - 1 + shared->dim_n_Tw_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, T[shared->dim_T_1 * (j - 1) + i - 1] - internal.n_T[shared->dim_n_T_1 * (j - 1) + i - 1], 1 - std::exp(shared->D[j - 1] * shared->dt));
      }
    }
    for (int i = 1; i <= shared->dim_n_Tx_1; ++i) {
      for (int j = 1; j <= shared->dim_n_Tx_2; ++j) {
        internal.n_Tx[i - 1 + shared->dim_n_Tx_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, internal.n_T[shared->dim_n_T_1 * (j - 1) + i - 1], shared->exr / (real_type) shared->r_T[shared->dim_r_T_1 * (j - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_Ux_1; ++i) {
      for (int j = 1; j <= shared->dim_n_Ux_2; ++j) {
        internal.n_Ux[i - 1 + shared->dim_n_Ux_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, internal.n_U[shared->dim_n_U_1 * (j - 1) + i - 1], shared->exr / (real_type) internal.r_U[shared->dim_r_U_1 * (j - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_lambda; ++i) {
      state_next[shared->offset_variable_lambda + i - 1] = shared->p[i - 1] * beta * (shared->epsilon * internal.prop_C[i - 1] + (1 - shared->epsilon) * odin_sum1<real_type>(internal.foi_LH.data(), 0, shared->dim_foi_LH));
    }
    ...
    for (int i = 1; i <= shared->dim_n_AT_1; ++i) {
      for (int j = 1; j <= shared->dim_n_AT_2; ++j) {
        internal.n_AT[i - 1 + shared->dim_n_AT_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, internal.n_A[shared->dim_n_A_1 * (j - 1) + i - 1] - internal.n_Ax[shared->dim_n_Ax_1 * (j - 1) + i - 1], internal.r_AT[shared->dim_r_AT_1 * (j - 1) + i - 1] / (real_type) (internal.r_AT[shared->dim_r_AT_1 * (j - 1) + i - 1] + shared->r_AU[shared->dim_r_AU_1 * (j - 1) + i - 1]));
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_UI_1; ++i) {
      for (int j = 1; j <= shared->dim_n_UI_2; ++j) {
        internal.n_UI[i - 1 + shared->dim_n_UI_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, internal.n_U[shared->dim_n_U_1 * (j - 1) + i - 1] - internal.n_Ux[shared->dim_n_Ux_1 * (j - 1) + i - 1], internal.r_UI[shared->dim_r_UI_1 * (j - 1) + i - 1] / (real_type) (internal.r_UI[shared->dim_r_UI_1 * (j - 1) + i - 1] + internal.r_UU[shared->dim_r_UU_1 * (j - 1) + i - 1]));
      }
    }
    for (int i = 1; i <= shared->dim_n_x; ++i) {
      internal.n_x[i - 1] = odin_sum2<real_type>(internal.n_Ux.data(), i - 1, i, 0, shared->dim_n_Ux_2, shared->dim_n_Ux_1) + odin_sum2<real_type>(internal.n_Ix.data(), i - 1, i, 0, shared->dim_n_Ix_2, shared->dim_n_Ix_1) + odin_sum2<real_type>(internal.n_Ax.data(), i - 1, i, 0, shared->dim_n_Ax_2, shared->dim_n_Ax_1) + odin_sum2<real_type>(internal.n_Sx.data(), i - 1, i, 0, shared->dim_n_Sx_2, shared->dim_n_Sx_1) + odin_sum2<real_type>(internal.n_Tx.data(), i - 1, i, 0, shared->dim_n_Tx_2, shared->dim_n_Tx_1);
    }
    ...
    for (int i = 1; i <= shared->dim_n_Uw_1; ++i) {
      for (int j = 1; j <= shared->dim_n_Uw_2; ++j) {
        internal.n_Uw[i - 1 + shared->dim_n_Uw_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, U[shared->dim_U_1 * (j - 1) + i - 1] - internal.n_UI[shared->dim_n_UI_1 * (j - 1) + i - 1] - internal.n_Ux[shared->dim_n_Ux_1 * (j - 1) + i - 1], 1 - std::exp(shared->D[j - 1] * shared->dt));
      }
    }
    for (int i = 1; i <= shared->dim_n_vod_1; ++i) {
      for (int j = 1; j <= shared->dim_n_vod_2; ++j) {
        internal.n_vod[i - 1 + shared->dim_n_vod_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, internal.n_TU[shared->dim_n_TU_1 * (j - 1) + i - 1], shared->u_vod[shared->dim_u_vod_1 * (j - 1) + i - 1] * vax_switch);
      }
    }
    for (int i = 1; i <= shared->dim_n_xU_1; ++i) {
      int j = 1;
      internal.n_xU[i - 1 + shared->dim_n_xU_1 * (j - 1)] = (shared->fix_pop == 0 ? dust::random::poisson<real_type>(rng_state, shared->enr * shared->q[i - 1] * shared->dt) : internal.n_x[i - 1]);
    }
    for (int i = 1; i <= shared->dim_A_1; ++i) {
      for (int j = 1; j <= shared->dim_A_2; ++j) {
        state_next[shared->offset_variable_A + i - 1 + shared->dim_A_1 * (j - 1)] = A[shared->dim_A_1 * (j - 1) + i - 1] + internal.n_IA[shared->dim_n_IA_1 * (j - 1) + i - 1] - internal.n_A[shared->dim_n_A_1 * (j - 1) + i - 1] + odin_sum3<real_type>(internal.wA.data(), i - 1, i, j - 1, j, 0, shared->dim_wA_3, shared->dim_wA_1, shared->dim_wA_12);
      }
    }
    for (int i = 1; i <= shared->dim_I_1; ++i) {
      for (int j = 1; j <= shared->dim_I_2; ++j) {
        state_next[shared->offset_variable_I + i - 1 + shared->dim_I_1 * (j - 1)] = I[shared->dim_I_1 * (j - 1) + i - 1] + internal.n_UI[shared->dim_n_UI_1 * (j - 1) + i - 1] - internal.n_I[shared->dim_n_I_1 * (j - 1) + i - 1] + odin_sum3<real_type>(internal.wI.data(), i - 1, i, j - 1, j, 0, shared->dim_wI_3, shared->dim_wI_1, shared->dim_wI_12);
      }
    }
    for (int i = 1; i <= shared->dim_S_1; ++i) {
      for (int j = 1; j <= shared->dim_S_2; ++j) {
        state_next[shared->offset_variable_S + i - 1 + shared->dim_S_1 * (j - 1)] = S[shared->dim_S_1 * (j - 1) + i - 1] + internal.n_IS[shared->dim_n_IS_1 * (j - 1) + i - 1] - internal.n_S[shared->dim_n_S_1 * (j - 1) + i - 1] + odin_sum3<real_type>(internal.wS.data(), i - 1, i, j - 1, j, 0, shared->dim_wS_3, shared->dim_wS_1, shared->dim_wS_12);
      }
    }
    for (int i = 1; i <= shared->dim_T_1; ++i) {
      for (int j = 1; j <= shared->dim_T_2; ++j) {
        state_next[shared->offset_variable_T + i - 1 + shared->dim_T_1 * (j - 1)] = T[shared->dim_T_1 * (j - 1) + i - 1] + internal.n_ST[shared->dim_n_ST_1 * (j - 1) + i - 1] + internal.n_AT[shared->dim_n_AT_1 * (j - 1) + i - 1] - internal.n_T[shared->dim_n_T_1 * (j - 1) + i - 1] + odin_sum3<real_type>(internal.wT.data(), i - 1, i, j - 1, j, 0, shared->dim_wT_3, shared->dim_wT_1, shared->dim_wT_12);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_diag_a_1; ++i) {
      for (int j = 1; j <= shared->dim_diag_a_2; ++j) {
        state_next[shared->offset_variable_diag_a + i - 1 + shared->dim_diag_a_1 * (j - 1)] = (fmodr<real_type>(step, shared->steps_per_year) == 0 ? internal.n_AT[shared->dim_n_AT_1 * (j - 1) + i - 1] : diag_a[shared->dim_diag_a_1 * (j - 1) + i - 1] + internal.n_AT[shared->dim_n_AT_1 * (j - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_diag_s_1; ++i) {
      for (int j = 1; j <= shared->dim_diag_s_2; ++j) {
        state_next[shared->offset_variable_diag_s + i - 1 + shared->dim_diag_s_1 * (j - 1)] = (fmodr<real_type>(step, shared->steps_per_year) == 0 ? internal.n_ST[shared->dim_n_ST_1 * (j - 1) + i - 1] : diag_s[shared->dim_diag_s_1 * (j - 1) + i - 1] + internal.n_ST[shared->dim_n_ST_1 * (j - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_incid_1; ++i) {
      for (int j = 1; j <= shared->dim_incid_2; ++j) {
        state_next[shared->offset_variable_incid + i - 1 + shared->dim_incid_1 * (j - 1)] = (fmodr<real_type>(step, shared->steps_per_year) == 0 ? internal.n_UI[shared->dim_n_UI_1 * (j - 1) + i - 1] : incid[shared->dim_incid_1 * (j - 1) + i - 1] + internal.n_UI[shared->dim_n_UI_1 * (j - 1) + i - 1]);
      }
    }
    state_next[2] = (fmodr<real_type>(step, shared->steps_per_year) == 0 ? odin_sum1<real_type>(internal.n_x.data(), 0, shared->dim_n_x) : leavers + odin_sum1<real_type>(internal.n_x.data(), 0, shared->dim_n_x));
    for (int i = 1; i <= shared->dim_treated_1; ++i) {
      for (int j = 1; j <= shared->dim_treated_2; ++j) {
        state_next[shared->offset_variable_treated + i - 1 + shared->dim_treated_1 * (j - 1)] = (fmodr<real_type>(step, shared->steps_per_year) == 0 ? internal.n_TU[shared->dim_n_TU_1 * (j - 1) + i - 1] : treated[shared->dim_treated_1 * (j - 1) + i - 1] + internal.n_TU[shared->dim_n_TU_1 * (j - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_vbe_1; ++i) {
      for (int j = 1; j <= shared->dim_n_vbe_2; ++j) {
        internal.n_vbe[i - 1 + shared->dim_n_vbe_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, internal.n_xU[shared->dim_n_xU_1 * (j - 1) + i - 1], shared->u_vbe[shared->dim_u_vbe_1 * (j - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_vos_1; ++i) {
      for (int j = 1; j <= shared->dim_n_vos_2; ++j) {
        internal.n_vos[i - 1 + shared->dim_n_vos_1 * (j - 1)] = dust::random::binomial<real_type>(rng_state, internal.n_UU[shared->dim_n_UU_1 * (j - 1) + i - 1], shared->u_vos[shared->dim_u_vos_1 * (j - 1) + i - 1] * vax_switch);
      }
    }
    ...
    state_next[1] = (fmodr<real_type>(step, shared->steps_per_year) == 0 ? odin_sum1<real_type>(internal.n_xU.data(), 0, shared->dim_n_xU) : entrants + odin_sum1<real_type>(internal.n_xU.data(), 0, shared->dim_n_xU));
    for (int i = 1; i <= shared->dim_screened_1; ++i) {
      for (int j = 1; j <= shared->dim_screened_2; ++j) {
        state_next[shared->offset_variable_screened + i - 1 + shared->dim_screened_1 * (j - 1)] = (fmodr<real_type>(step, shared->steps_per_year) == 0 ? internal.n_UU[shared->dim_n_UU_1 * (j - 1) + i - 1] : screened[shared->dim_screened_1 * (j - 1) + i - 1] + internal.n_UU[shared->dim_n_UU_1 * (j - 1) + i - 1]);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_vaccinated_1; ++i) {
      for (int j = 1; j <= shared->dim_vaccinated_2; ++j) {
        state_next[shared->offset_variable_vaccinated + i - 1 + shared->dim_vaccinated_1 * (j - 1)] = (fmodr<real_type>(step, shared->steps_per_year) == 0 ? internal.n_v[shared->dim_n_v_12 * (j - 1) + shared->dim_n_v_1 * (j - 1) + i - 1] : vaccinated[shared->dim_vaccinated_1 * (j - 1) + i - 1] + internal.n_v[shared->dim_n_v_12 * (j - 1) + shared->dim_n_v_1 * (j - 1) + i - 1]);
      }
    }
  }
private:
...
dust::pars_type<model> dust_pars<model>(cpp11::list user) {
  ...
  shared->dt = 1 / (real_type) shared->steps_per_year;
  internal.Np = std::vector<real_type>(shared->dim_Np);
  internal.foi_LH = std::vector<real_type>(shared->dim_foi_LH);
  shared->initial_eta = std::vector<real_type>(shared->dim_eta);
  shared->initial_lambda = std::vector<real_type>(shared->dim_lambda);
  internal.n_x = std::vector<real_type>(shared->dim_n_x);
  internal.prop_C = std::vector<real_type>(shared->dim_prop_C);
  ...
  shared->enr = user_get_scalar<real_type>(user, "enr", shared->enr, NA_REAL, NA_REAL);
  shared->epsilon = user_get_scalar<real_type>(user, "epsilon", shared->epsilon, NA_REAL, NA_REAL);
  std::array <int, 1> dim_eta_h_step;
  shared->eta_h_step = user_get_array_variable<real_type, 1>(user, "eta_h_step", shared->eta_h_step, dim_eta_h_step, NA_REAL, NA_REAL);
  shared->dim_eta_h_step = shared->eta_h_step.size();
  std::array <int, 1> dim_eta_l_step;
  shared->eta_l_step = user_get_array_variable<real_type, 1>(user, "eta_l_step", shared->eta_l_step, dim_eta_l_step, NA_REAL, NA_REAL);
  shared->dim_eta_l_step = shared->eta_l_step.size();
  shared->exr = user_get_scalar<real_type>(user, "exr", shared->exr, NA_REAL, NA_REAL);
  shared->fix_pop = user_get_scalar<real_type>(user, "fix_pop", shared->fix_pop, NA_REAL, NA_REAL);
  shared->mu = user_get_scalar<real_type>(user, "mu", shared->mu, NA_REAL, NA_REAL);
  shared->n_vax = user_get_scalar<int>(user, "n_vax", shared->n_vax, NA_REAL, NA_REAL);
  shared->nu = user_get_scalar<real_type>(user, "nu", shared->nu, NA_REAL, NA_REAL);
  shared->psi = user_get_scalar<real_type>(user, "psi", shared->psi, NA_REAL, NA_REAL);
  ...
  shared->vax_step = user_get_array_variable<real_type, 1>(user, "vax_step", shared->vax_step, dim_vax_step, NA_REAL, NA_REAL);
  ...
  shared->D = user_get_array_fixed<real_type, 1>(user, "D", shared->D, {shared->dim_D}, NA_REAL, NA_REAL);
  ...
  shared->p = user_get_array_fixed<real_type, 1>(user, "p", shared->p, {shared->dim_p}, NA_REAL, NA_REAL);
  shared->q = user_get_array_fixed<real_type, 1>(user, "q", shared->q, {shared->dim_q}, NA_REAL, NA_REAL);
  shared->vea = user_get_array_fixed<real_type, 1>(user, "vea", shared->vea, {shared->dim_vea}, NA_REAL, NA_REAL);
  shared->ved = user_get_array_fixed<real_type, 1>(user, "ved", shared->ved, {shared->dim_ved}, NA_REAL, NA_REAL);
  shared->vei = user_get_array_fixed<real_type, 1>(user, "vei", shared->vei, {shared->dim_vei}, NA_REAL, NA_REAL);
  shared->ves = user_get_array_fixed<real_type, 1>(user, "ves", shared->ves, {shared->dim_ves}, NA_REAL, NA_REAL);
  shared->A0 = user_get_array_fixed<real_type, 2>(user, "A0", shared->A0, {shared->dim_A0_1, shared->dim_A0_2}, NA_REAL, NA_REAL);
  shared->I0 = user_get_array_fixed<real_type, 2>(user, "I0", shared->I0, {shared->dim_I0_1, shared->dim_I0_2}, NA_REAL, NA_REAL);
  shared->S0 = user_get_array_fixed<real_type, 2>(user, "S0", shared->S0, {shared->dim_S0_1, shared->dim_S0_2}, NA_REAL, NA_REAL);
  shared->T0 = user_get_array_fixed<real_type, 2>(user, "T0", shared->T0, {shared->dim_T0_1, shared->dim_T0_2}, NA_REAL, NA_REAL);
  shared->U0 = user_get_array_fixed<real_type, 2>(user, "U0", shared->U0, {shared->dim_U0_1, shared->dim_U0_2}, NA_REAL, NA_REAL);
  internal.C = std::vector<real_type>(shared->dim_C);
  ...
  shared->initial_cum_diag_a = std::vector<real_type>(shared->dim_cum_diag_a);
  shared->initial_cum_diag_s = std::vector<real_type>(shared->dim_cum_diag_s);
  shared->initial_cum_incid = std::vector<real_type>(shared->dim_cum_incid);
  shared->initial_cum_screened = std::vector<real_type>(shared->dim_cum_screened);
  shared->initial_cum_treated = std::vector<real_type>(shared->dim_cum_treated);
  shared->initial_cum_vaccinated = std::vector<real_type>(shared->dim_cum_vaccinated);
  shared->initial_diag_a = std::vector<real_type>(shared->dim_diag_a);
  shared->initial_diag_s = std::vector<real_type>(shared->dim_diag_s);
  shared->initial_incid = std::vector<real_type>(shared->dim_incid);
  shared->initial_screened = std::vector<real_type>(shared->dim_screened);
  shared->initial_treated = std::vector<real_type>(shared->dim_treated);
  shared->initial_vaccinated = std::vector<real_type>(shared->dim_vaccinated);
  internal.n_A = std::vector<real_type>(shared->dim_n_A);
  internal.n_AT = std::vector<real_type>(shared->dim_n_AT);
  internal.n_AU = std::vector<real_type>(shared->dim_n_AU);
  internal.n_Aw = std::vector<real_type>(shared->dim_n_Aw);
  internal.n_Ax = std::vector<real_type>(shared->dim_n_Ax);
  internal.n_I = std::vector<real_type>(shared->dim_n_I);
  internal.n_IA = std::vector<real_type>(shared->dim_n_IA);
  internal.n_IS = std::vector<real_type>(shared->dim_n_IS);
  internal.n_Iw = std::vector<real_type>(shared->dim_n_Iw);
  internal.n_Ix = std::vector<real_type>(shared->dim_n_Ix);
  internal.n_S = std::vector<real_type>(shared->dim_n_S);
  internal.n_ST = std::vector<real_type>(shared->dim_n_ST);
  internal.n_Sw = std::vector<real_type>(shared->dim_n_Sw);
  internal.n_Sx = std::vector<real_type>(shared->dim_n_Sx);
  internal.n_T = std::vector<real_type>(shared->dim_n_T);
  internal.n_TU = std::vector<real_type>(shared->dim_n_TU);
  internal.n_Tw = std::vector<real_type>(shared->dim_n_Tw);
  internal.n_Tx = std::vector<real_type>(shared->dim_n_Tx);
  internal.n_U = std::vector<real_type>(shared->dim_n_U);
  internal.n_UI = std::vector<real_type>(shared->dim_n_UI);
  internal.n_UU = std::vector<real_type>(shared->dim_n_UU);
  internal.n_Uw = std::vector<real_type>(shared->dim_n_Uw);
  internal.n_Ux = std::vector<real_type>(shared->dim_n_Ux);
  internal.n_v = std::vector<real_type>(shared->dim_n_v);
  internal.n_vbe = std::vector<real_type>(shared->dim_n_vbe);
  internal.n_vod = std::vector<real_type>(shared->dim_n_vod);
  internal.n_vos = std::vector<real_type>(shared->dim_n_vos);
  internal.n_xU = std::vector<real_type>(shared->dim_n_xU);
  internal.r_A = std::vector<real_type>(shared->dim_r_A);
  internal.r_AT = std::vector<real_type>(shared->dim_r_AT);
  ...
  shared->r_ST = std::vector<real_type>(shared->dim_r_ST);
  shared->r_T = std::vector<real_type>(shared->dim_r_T);
  shared->r_TU = std::vector<real_type>(shared->dim_r_TU);
  internal.r_U = std::vector<real_type>(shared->dim_r_U);
  internal.r_UI = std::vector<real_type>(shared->dim_r_UI);
  internal.r_UU = std::vector<real_type>(shared->dim_r_UU);
  internal.wA = std::vector<real_type>(shared->dim_wA);
  internal.wI = std::vector<real_type>(shared->dim_wI);
  internal.wS = std::vector<real_type>(shared->dim_wS);
  internal.wT = std::vector<real_type>(shared->dim_wT);
  internal.wU = std::vector<real_type>(shared->dim_wU);
  for (int i = 1; i <= shared->dim_cum_diag_a_1; ++i) {
    for (int j = 1; j <= shared->dim_cum_diag_a_2; ++j) {
  ...
  shared->offset_variable_A = shared->dim_I + shared->dim_U + shared->dim_eta + shared->dim_lambda + 4;
  shared->offset_variable_I = shared->dim_U + shared->dim_eta + shared->dim_lambda + 4;
  shared->offset_variable_N = shared->dim_A + shared->dim_I + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_cum_diag_a + shared->dim_cum_diag_s + shared->dim_cum_incid + shared->dim_cum_screened + shared->dim_cum_treated + shared->dim_cum_vaccinated + shared->dim_diag_a + shared->dim_diag_s + shared->dim_eta + shared->dim_incid + shared->dim_lambda + shared->dim_screened + shared->dim_treated + shared->dim_vaccinated + 4;
  shared->offset_variable_S = shared->dim_A + shared->dim_I + shared->dim_U + shared->dim_eta + shared->dim_lambda + 4;
  shared->offset_variable_T = shared->dim_A + shared->dim_I + shared->dim_S + shared->dim_U + shared->dim_eta + shared->dim_lambda + 4;
  shared->offset_variable_cum_diag_a = shared->dim_A + shared->dim_I + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_cum_incid + shared->dim_eta + shared->dim_lambda + 4;
  shared->offset_variable_cum_diag_s = shared->dim_A + shared->dim_I + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_cum_diag_a + shared->dim_cum_incid + shared->dim_eta + shared->dim_lambda + 4;
  shared->offset_variable_cum_incid = shared->dim_A + shared->dim_I + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_eta + shared->dim_lambda + 4;
  shared->offset_variable_cum_screened = shared->dim_A + shared->dim_I + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_cum_diag_a + shared->dim_cum_diag_s + shared->dim_cum_incid + shared->dim_cum_treated + shared->dim_eta + shared->dim_lambda + 4;
  shared->offset_variable_cum_treated = shared->dim_A + shared->dim_I + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_cum_diag_a + shared->dim_cum_diag_s + shared->dim_cum_incid + shared->dim_eta + shared->dim_lambda + 4;
  shared->offset_variable_cum_vaccinated = shared->dim_A + shared->dim_I + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_cum_diag_a + shared->dim_cum_diag_s + shared->dim_cum_incid + shared->dim_cum_screened + shared->dim_cum_treated + shared->dim_eta + shared->dim_lambda + 4;
  shared->offset_variable_diag_a = shared->dim_A + shared->dim_I + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_cum_diag_a + shared->dim_cum_diag_s + shared->dim_cum_incid + shared->dim_cum_screened + shared->dim_cum_treated + shared->dim_cum_vaccinated + shared->dim_eta + shared->dim_incid + shared->dim_lambda + 4;
  shared->offset_variable_diag_s = shared->dim_A + shared->dim_I + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_cum_diag_a + shared->dim_cum_diag_s + shared->dim_cum_incid + shared->dim_cum_screened + shared->dim_cum_treated + shared->dim_cum_vaccinated + shared->dim_diag_a + shared->dim_eta + shared->dim_incid + shared->dim_lambda + 4;
  shared->offset_variable_incid = shared->dim_A + shared->dim_I + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_cum_diag_a + shared->dim_cum_diag_s + shared->dim_cum_incid + shared->dim_cum_screened + shared->dim_cum_treated + shared->dim_cum_vaccinated + shared->dim_eta + shared->dim_lambda + 4;
  shared->offset_variable_screened = shared->dim_A + shared->dim_I + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_cum_diag_a + shared->dim_cum_diag_s + shared->dim_cum_incid + shared->dim_cum_screened + shared->dim_cum_treated + shared->dim_cum_vaccinated + shared->dim_diag_a + shared->dim_diag_s + shared->dim_eta + shared->dim_incid + shared->dim_lambda + shared->dim_treated + 4;
  shared->offset_variable_treated = shared->dim_A + shared->dim_I + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_cum_diag_a + shared->dim_cum_diag_s + shared->dim_cum_incid + shared->dim_cum_screened + shared->dim_cum_treated + shared->dim_cum_vaccinated + shared->dim_diag_a + shared->dim_diag_s + shared->dim_eta + shared->dim_incid + shared->dim_lambda + 4;
  shared->offset_variable_vaccinated = shared->dim_A + shared->dim_I + shared->dim_S + shared->dim_T + shared->dim_U + shared->dim_cum_diag_a + shared->dim_cum_diag_s + shared->dim_cum_incid + shared->dim_cum_screened + shared->dim_cum_treated + shared->dim_cum_vaccinated + shared->dim_diag_a + shared->dim_diag_s + shared->dim_eta + shared->dim_incid + shared->dim_lambda + shared->dim_screened + shared->dim_treated + 4;
  shared->u_vbe = user_get_array_fixed<real_type, 2>(user, "u_vbe", shared->u_vbe, {shared->dim_u_vbe_1, shared->dim_u_vbe_2}, NA_REAL, NA_REAL);
  shared->u_vod = user_get_array_fixed<real_type, 2>(user, "u_vod", shared->u_vod, {shared->dim_u_vod_1, shared->dim_u_vod_2}, NA_REAL, NA_REAL);
  shared->u_vos = user_get_array_fixed<real_type, 2>(user, "u_vos", shared->u_vos, {shared->dim_u_vos_1, shared->dim_u_vos_2}, NA_REAL, NA_REAL);
  shared->vbe = user_get_array_fixed<real_type, 3>(user, "vbe", shared->vbe, {shared->dim_vbe_1, shared->dim_vbe_2, shared->dim_vbe_3}, NA_REAL, NA_REAL);
  shared->vod = user_get_array_fixed<real_type, 3>(user, "vod", shared->vod, {shared->dim_vod_1, shared->dim_vod_2, shared->dim_vod_3}, NA_REAL, NA_REAL);
  shared->vos = user_get_array_fixed<real_type, 3>(user, "vos", shared->vos, {shared->dim_vos_1, shared->dim_vos_2, shared->dim_vos_3}, NA_REAL, NA_REAL);
  shared->w = user_get_array_fixed<real_type, 2>(user, "w", shared->w, {shared->dim_w_1, shared->dim_w_2}, NA_REAL, NA_REAL);
  for (int i = 1; i <= shared->dim_A_1; ++i) {
    for (int j = 1; j <= shared->dim_A_2; ++j) {
  ...
  for (int i = 1; i <= shared->dim_r_AU_1; ++i) {
    for (int j = 1; j <= shared->dim_r_AU_2; ++j) {
      shared->r_AU[i - 1 + shared->dim_r_AU_1 * (j - 1)] = shared->nu / (real_type) (1 - shared->ved[j - 1]);
    }
  }
